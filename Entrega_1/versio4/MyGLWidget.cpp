...
void MyGLWidget::modelTransformArbre (const glm::vec3& posicio, float mida)
{
  // Codi per a la TG necessària
	...
	TG = glm::scale(TG,glm::vec3(midaModel,midaModel,0.)); //per tal de que l'alcada sigui 1
	TG = glm::scale(TG,glm::vec3(mida,mida,0.));
	TG= glm::translate(TG,glm::vec3(0.,offsetBase,0.)); //per fer que la base del tronc es posi al centre
...
void MyGLWidget::creaBuffersCopaQ ()
{
  glm::vec3 Vertices[4];  // Quatre vèrtexs amb X, Y i Z
  Vertices[0] = glm::vec3(-0.5,  -0.2, 0);
  Vertices[1] = glm::vec3( 0.5,  -0.2, 0);
  Vertices[2] = glm::vec3( 0.5,  0.8, 0);
  Vertices[3] = glm::vec3(-0.5,  0.8, 0);

  // Creació del Vertex Array Object (VAO) que usarem per pintar el tronc
  glGenVertexArrays(1, &VAOCopaQ);
  glBindVertexArray(VAOCopaQ);
